  ...
  const handleFile = useCallback(
    async (file: File) => {
      ...
      try {
        ...
        const { data } = await api.post('/upload/parse-resume', formData, {
        ...
        // Store parsed data so the builder can pre-fill it
        ...
        setTimeout(() => {
          router.push(`/builder/${selectedTemplate}?source=upload`);
        }, 800);
      } catch (err: any) {
        setState('error');
        setError(
      ...
      <div className="container">
        ...
        <div className="upload-page__header">
          ...
          <p className="upload-page__subtitle">
            Upload your existing resume as a PDF or image. We&apos;ll extract your
            information and rebuild it in any template — instantly.
          </p>
        </div>
        ...
        <div className="upload-how">
          <p className="upload-how__title">How it works</p>
          <div className="upload-how__steps">
            {[
              ...
              <div key={s.t} className="upload-how__step">
                <span className="upload-how__step-icon">{s.icon}</span>
                <strong>{s.t}</strong>
                <span>{s.d}</span>
              </div>
            ))}
